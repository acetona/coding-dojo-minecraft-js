...
exports.caracteresVersAscii = function(caracteres) {
  ...
  // Developpeur : ta tache commence ici !
  var lettres = "abcdefghijklmnopqrstuvwxyz ";
  var reponse = alphabet;
  if (caracteres !== '') {
    reponse = ['','','','',''];
    for(var lettre = 0; lettre < caracteres.length; lettre++) {
      for (var hauteur = 0; hauteur < 5; hauteur++) {
        if(lettres.indexOf(caracteres[lettre].toLowerCase()) == -1){
            reponse[hauteur]+=alphabet[hauteur].substr(0, 4);
        }else{
          reponse[hauteur]+=alphabet[hauteur].substr(lettres.indexOf(caracteres[lettre].toLowerCase())*4, 4);
          }
      }
    }
  }
